 ...
 * @author parastoo
 */
public class F43_Inner_CenteredSim extends EntityBasedFeature {
    public F43_Inner_CenteredSim(String dataset, String rs, int docCount, int start, int end, String feature) {
        ...
        try {
            ...
            for (int i = 0; i < docsTitle.size(); i++) {
                for (int j = i+1; j < docsTitle.size(); j++) {
                    ...
                    p_d_d += calculateCenteredSimilarity(doc1,docsTitle.get(i), docsTitle.get(j));
                    docsScore.add(p_d_d);
                    comparisonCount++;
                }
            }
